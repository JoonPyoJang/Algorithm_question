...
생각해보기
처음 시작 이후 시작 지점을 찾고 배열로 만들어 이동을 시킨다
우선 배열로 만들고 S를 찾아 그 좌표부터 시작
스타트 포인트를 찾고 장해물은 리스트로 정리한다.
스타트 포인트에서 이동 시 if 문으로 이동을 분류하고 limit 를 넘어가면 실행하지 않는다.
이동할때 장해물을 확인해야 하는데 이동전 이동위치에 장애물이 있는지 확인
'''
